...
'        <a href="' + root + 'writings/">Writings &amp; Reviews</a>',
'        <a href="' + site.origin + '" rel="nofollow">Original Palmer Seminary site</a>',
'      </div>',
'    </div>',
...
'      <h2>A lifetime of reading the Hebrew Bible against its Semitic and Arabic cognates</h2>',
'    </div>',
'    <div class="volumes">',
vols,
'    </div>',
...
'    </div>',
'    <a class="callout" href="' + pdfHref(root, D.booksIntro.pdf) + '" style="display:block;margin-bottom:2.5rem;text-decoration:none">',
'      <p class="eyebrow" style="margin-bottom:.4rem">Start here</p>',
'      <h2 style="font-size:1.4rem;margin:0 0 .3rem"><span class="ilink">' + esc(D.booksIntro.title) + '</span></h2>',
'      <p class="muted" style="margin:0">' + esc(D.booksIntro.subtitle) + ' &middot; PDF</p>',
'    </a>',
'    <div class="volumes">',
list,
'    </div>',
...
'      <p class="lede narrow">A working set of Semitic lexicons and special collections. These large reference works remain on Dr. McDaniel&rsquo;s original Palmer Seminary website; each link opens there in full.</p>',
'    </div>',
'    <div class="callout narrow" style="margin-bottom:2.5rem">',
'      <p style="margin:0"><strong>About the original site.</strong> Dr. McDaniel&rsquo;s original website is hosted by Palmer Theological Seminary at <a class="ilink" href="' + site.origin + '" rel="nofollow">tmcdaniel.palmerseminary.edu</a>. It was last updated on <strong>3 January 2014</strong>. Its &ldquo;Internet Links for Clergy&rdquo; directory dates largely to the 1990s and early 2000s, and most of those external links no longer work in 2026 &mdash; so that directory was <em>not</em> carried over to this archive. Everything Dr. McDaniel wrote is preserved here.</p>',
'    </div>',
'    <div class="cards">',
cards,
'    </div>',
...
function buildAbout(){
  ...
  // Retirement-interview video — own section, renders only when video.id is set
  var V = D.video;
  var videoSection = '';
  if(V && V.id){
    var vTitle = 'Thomas F. McDaniel — ' + V.title + ' (' + V.year + ')';
    ld.push({
      '@context':'https://schema.org','@type':'VideoObject','name':vTitle,
      'description':V.desc,'uploadDate':V.uploadDate,
    ...
    videoSection = [
'  <hr class="rule">',
'  <section class="wrap" id="interview">',
